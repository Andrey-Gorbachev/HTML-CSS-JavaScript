...
const getGoods = async () => { 	
	...
	const result = await fetch('db/db.json'); // result это промисс
		...
		if (goodItem) {
			...
			getGoods()
				...
				.then( ({id, name, price}) => { // деструткурируем
					...
					this.countQuantity();
					console.log(this.cartGoods); // здесь ! синхронно показывает
				});
				//console.log(this.cartGoods); // здесь не синхронно показывает
		}
